...
class NewsFragment : Fragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    val newsAdapter = NewsAdapter {
      when (it.isBookmarked) {
        true -> viewModel.deleteNews(it)
        false -> viewModel.saveNews(it)
      }
    }
    ...
    when (tabName) {
        ...
        viewModel.getHeadlineNews().observe(viewLifecycleOwner) {
          ...
          when (it) {
            ...
            is Result.Success<*> -> {
              ...
              newsAdapter.submitList(newsData as MutableList<NewsEntity>?)
            }
            is Result.Error -> {
              binding?.progressBar?.visibility = View.GONE
      ...
      TAB_BOOKMARK -> {
        viewModel.getBookmarkNews().observe(viewLifecycleOwner) {
          binding?.progressBar?.visibility = View.GONE
